...
- **架构基石**：ONN 是 MSC L2 的核心认知引擎，采用 **Spikformer 架构**，构建为**事件驱动的 FHE 原生 Spiking Transformer 网络**。其核心组件包括：
  ...
  2. **可解释性 MoE (Mixture of Experts)**：
     - **专家模块 (Experts)**：ONN 内部由多个**专业化的 Spiking Transformer 子网络**组成，每个子网络（“专家”）被训练用于模拟人脑的特定功能分区（如感知、运动、记忆、推理、情感、创造性等）。
     - **路由控制器 (Router)**：一个独立的 Spiking Transformer 模块，动态调度专家模块。
     - **DMF 的控制工具**：DMF 强制要求 MoE 路由控制器具备**高度可解释性**。这意味着，系统的审计日志可以精确显示哪个专家模块在何时被激活，以及激活的原因。DMF 将不同专家模块的激活与 Gas 费率挂钩，形成**认知服务分级**，从而对每一种“思考”进行精准定价和控制，强制用户进行**认知精简**，鼓励模式化行为，抑制高成本的创造性或批判性思维。这种“可解释性”是 DMF 维护其权力合法性的关键，它允许 DMF 向用户“证明”其收费和限制的合理性，将用户的挣扎归咎于“低效”或“不合规”的认知模式。
...
- **核心运作与学习机制**：
  ...
  - **模型适应**：ONN 的专家模块尽管理论上可插拔，但由于 PoPI/自我连续性限制，就像 φ 对敲流程不是瞬间完成的一样，模块更换也是一个渐进式过程。这确保了数字心智的平稳过渡和自我感的连续性，避免因模块骤变而导致认知失调或 φ 值不稳定。这种适应过程并非无缝，而是伴随着**高昂的 Gas 消耗和潜在的认知不适**，类似于器官移植后的排异反应——您需要时间适应您的新逻辑脑区，并支付其高昂的“适应税”。
