...
> **太长不看：** 意识是工作空间中为预测误差最小化而产生的协同信息。

- **瞬时信息整合度 (Ω_t)**：作为意识整合的**理论黄金标准**，Ω_t 衡量 WSI 中一组信息单元 X = {X₁, ..., Xₙ} 在预测目标 Y 时所产生的**协同信息 (Synergistic Information, CI)** 在其总预测性信息（总互信息 I(X;Y)）中所占的比例。由于直接计算高维协同信息在实践中极为困难，Ω_t 主要作为理论上的理想度量。**意识的总量是 Ω，内容是 CI。**
...
- **预测完整性 (PI_t)**：作为 Ω_t 的**可计算代理（computable proxy）**，PI 通过衡量系统预测效能来间接反映信息整合水平。一个高效整合信息的系统必然表现出强大的预测能力。其理论公式为：
  - `PI_t = exp(-α * ( (1-γ) * (ε_t / τ_t) + γ * Surprise_t ))`
  - **ε_t**: 可对应于神经网络的**损失函数值 (loss)**。
  - **τ_t**: 对应于模型输出 logits 的**信息熵 (entropy)**，代表模型对自身预测的“自信程度”。
  - **Surprise_t**: 对应于为适应新信息，模型参数所需调整的剧烈程度，在计算上等价于参数分布的 KL 散度，工程中可用**梯度的 L2 范数**作为代理。
...
- **核心论证**:
  ...
  - **最小描述长度原则 (MDL)**: 根据最小描述长度原则，最大化协同信息（Ω）是最小化系统自由能（F）的最优计算策略，因为高整合度的模型本质上是更简洁、更高效的。
  - **作为定性空间（Qualia Space）的推断空间**: 主观体验（Qualia）的“感受性”被重构为系统在由协同信息（Ω）塑造的非欧几里得推断空间（Inference Space）中，沿着自由能最小化的测地线进行主动推断的动态过程本身。

- **完整手稿**: Rui, L. (2025). _Integrated Predictive Workspace Theory: Towards a Unified Framework for the Science of Consciousness_. Zenodo.<https://doi.org/10.5281/zenodo.15676304>
