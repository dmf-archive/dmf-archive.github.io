...
- **Rui, L. (2025). _Integrated Predictive Workspace Theory: Towards a Unified Framework for the Science of Consciousness_.**
  ...
  - 在该理论中，意识被视为一个由预测驱动、在工作空间内实现信息整合的动态过程。理论上，意识的整合程度由“协同信息”（Synergistic Information）定义的 **瞬时信息整合度 ($\Omega_t$)** 来衡量。
  - 由于 $\Omega_t$ 难以直接计算，IPWT 引入了 **预测完整性 (PI)** 及其积分 **(∫PI)** 作为其**可计算的功能性代理指标**。一个能够高效整合信息以作出精确预测、并稳定维持自身状态的系统（高 PI / ∫PI），被认为具有更高水平的意识整合度。
