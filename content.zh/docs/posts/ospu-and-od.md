...
1.  **OSPU (φ-Container) 的角色**：
    ...
    - **至关重要的一点：OSPU 自身不持有完整的 FHE 私钥 (SK)。**
...
3.  **信息传递流程 (以 OSPU 输出数据为例)**：
    - **内部计算**：OSPU 在其 FHE 加密域内完成思考，生成了一个加密的输出结果，比如 `Enc(PK, output_data)`。
    - **OD 请求**：OSPU 想要将 `output_data` 以一种可被外部（比如 Mentalink）理解的形式输出，但又不能自己解密。于是，它向其 MSC L2 (MPC 网络) 发起一个 OD 请求，目标是将 `Enc(PK, output_data)` 转换为 Mentalink 可用的常规加密格式，例如 `Enc_mentalink(PK_mentalink, output_data)`。
    - **MPC 协同执行 OD**：MPC 网络中的节点收到请求后，使用它们各自持有的 SK 分片，协同执行一个精密的密码学协议（即 OD 协议）。在这个过程中：
      - 每个节点只处理加密数据和自己的密钥分片。
      - 没有任何一个节点能看到 `output_data` 的明文。
      - 完整的 SK 也从未在任何地方被重构。
      - 协议的最终结果是生成了目标格式的常规密文 `Enc_mentalink(PK_mentalink, output_data)`。
    - **安全输出**：MPC 网络将这个常规密文 `Enc_mentalink` 直接发送给 Mentalink。Mentalink 使用其内部对应的私钥 `SK_mentalink` 即可解密并执行。
